...
describe('lei-udp', function () {
  ...
  it('send simple data', function (done) {
    ...
    address.pocketLossProb = POCKET_LOSS_PROB;
    var s, c1, c2;
    ...
    async.series([
      ...
      function (next) {
        // 客户端发送数据
        c1 = support.createSocket({pocketLossProb: POCKET_LOSS_PROB});
      ...
      function (next) {
        // 客户端连接
        c2 = support.createSocket({pocketLossProb: POCKET_LOSS_PROB});
  ...
  it('send big data', function (done) {
    ...
    async.series([
      ...
      function (next) {
        // 客户端发送数据
        c1 = support.createSocket({maxMessageSize: 1500, pocketLossProb: POCKET_LOSS_PROB});
      ...
      function (next) {
        // 客户端连接
        c2 = support.createSocket({maxMessageSize: 3000, pocketLossProb: POCKET_LOSS_PROB});
        c2.sendR(address.host, address.port, msgBuf2);
        c2.sendR(address.host, address.port, msgBuf2);
        c2.sendR(address.host, address.port, msgBuf2);
        c2.sendR(address.host, address.port, msgBuf2);
