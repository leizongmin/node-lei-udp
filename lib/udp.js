...
UDP.prototype._bindSocketEvents = function () {
  ...
  function getCacheList (addr, session) {
    var caches = self._getRemoteNamespace(addr.host, addr.port, 'cache', {});
    if (!caches[session]) caches[session] = {size: 0, received: 0, list: []};
    return caches[session];
  }

  function checkCacheList (addr, cache, session) {
    if (cache.size > 0 && cache.received === cache.size) {
      deleteCache(addr, session);
      self.emit('data', addr, utils.concatBuffer(cache.list));
    }
  }

  function deleteCache (addr, session) {
    var caches = self._getRemoteNamespace(addr.host, addr.port, 'cache', {});
  ...
  function onActionMulti (buf, addr) {
    ...
    var cache = getCacheList(addr, msg.session);
    if (!cache.list[msg.index]) cache.received++;
    cache.list[msg.index] = msg.data;
    checkCacheList(addr, cache, msg.session);
  }
...
UDP.prototype._send = function (isReliable, host, port, data, callback) {
  ...
  var dataList = utils.splitBuffer(data, self._options.maxMessageSize);
  var action = (dataList.length === 1 ? proto.SINGLE : proto.MULTI);
...
UDP.prototype._sendSingle = function (host, port, data, callback) {
  ...
  this._socket.send(buf, 0, buf.length, port, host, callback);
};
