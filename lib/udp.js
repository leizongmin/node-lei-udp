...
// 组装数据包
function packMessage (action, session, index, timestamp, data) {
  var buf = new Buffer(PACK_HEADER_SIZE + data.length);
  buf.writeUInt8(action, 0);
  buf.writeUIntBE(session, 1, 3);
  buf.writeUInt32BE(index, 4);
  buf.writeUInt32BE(timestamp, 8);
  data.copy(buf, PACK_HEADER_SIZE);
  return buf;
}
...
function unpackMessage (buf) {
  return {
    ...
    data: buf.slice(PACK_HEADER_SIZE)
  };
}
...
UDP.prototype._bindSocketEvents = function () {
  ...
  server.on('message', function (buf, addr) {
    ...
    self._debug('received %d bytes from %s:%d, action=%s', buf.length, addr.address, addr.port, info.action);
    switch (info.action) {

      case ACTION_SEND_SINGLE:
        receivedSingleData(false, buf, addr);
        break;

      case ACTION_SEND_SINGLE_SAFE:
        receivedSingleData(true, buf, addr);
        break;

      case ACTION_SEND_MULTI:
        receivedMultiData(false, buf, addr);
        break;

      case ACTION_SEND_MULTI_SAFE:
        receivedMultiData(true, buf, addr);
        break;

      case ACTION_SEND_MULTI_END:
        receivedMultiDataEnd(false, buf, addr);
        break;

      case ACTION_SEND_MULTI_SAFE_END:
        receivedMultiDataEnd(true, buf, addr);
        break;

      case ACTION_RESPONSE_SINGLE:
        receviedResponseSingle(buf, addr);
        break;

      case ACTION_RESPONSE_MULTI:
        receviedResponseMulti(buf, addr);
        break;
...
UDP.prototype._send = function (isSafe, host, port, data, callback) {
  ...
  async.eachSeries(dataList, function (data, next) {
    ...
    var buf = packMessage(action, session, index, timestamp, data);
