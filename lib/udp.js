...
UDP.prototype._startCleanCache = function () {
  ...
  self._setInterval(function () {
    ...
    for (var key in self._remoteNamespace) {
        ...
        for (var session in remote.sentSingle) {
          ...
          if (item.timestamp < responseTimeout) {
            ...
            if (item.retry < maxRetry) {
              ...
              socket.send(buf, 0, buf.length, addr.port, addr.host);
              debug('resend single: key=%s, session=%s', key, session);
            } else {
              // 超过重试次数
            ...
            for (var i = 0; i < item.list.length; i++) {
              if (item.list[i] && item.list[i].timestamp < responseTimeout) {
                if (item.list[i].retry < maxRetry) {
                  ...
                  socket.send(buf, 0, buf.length, addr.port, addr.host);
                  debug('resend multi: key=%s, session=%s, index=%s', key, session, i);
                } else {
                  // 超过重试次数
                  canceledMulti();
                  break;
      ...
      if (remote.sessionEmitted) {
        for (var session in remote.sessionEmitted) {
          if (remote.sessionEmitted[session] < cacheTimeout) {
            debug('clean sessionEmitted: key=%s, session=%s', key, session);
            delete remote.sessionEmitted[session];
          }
        }
      }
...
UDP.prototype._bindSocketEvents = function () {
  ...
  function onActionPing (buf, addr) {
    ...
    self.emit('ping', addr, msg.timestamp);
  }
  ...
  function checkReceivedCacheList (addr, cache, session) {
    if (cache.size > 0 && cache.received === cache.size) {
      ...
      emitDataBySession(addr, session, utils.concatBuffer(cache.list));
      return true;
    }
  }
  ...
  function emitDataBySession (addr, session, data) {
    var caches = self._getRemoteNamespace(addr.host, addr.port, 'sessionEmitted', {});
    // 保证即使重复收到一条消息也不会重复emit data
    if (!caches[session]) {
      caches[session] = Date.now();
      self.emit('data', addr, data);
    }
  }
  ...
  function onActionSingleReliable (buf, addr) {
    ...
    emitDataBySession(addr, msg.session, msg.data);
  ...
  function checkSentCacheListReliable (sentMulti, info, session, isAll) {
  ...
  function onActionMultiConfirmed (buf, addr) {
    ...
    if (info && info.list[msg.index]) {
      if (info.list[msg.index]) {
        ...
        info.confirmed++;
        checkSentCacheListReliable(sentMulti, info, msg.session);
      }
    }
  }
  ...
  function onActionMultiAllConfirmed (buf, addr) {
    ...
    checkSentCacheListReliable(sentMulti, info, msg.session, true);
  }
};

UDP.prototype.send = function (host, port, data, callback) {
  this._debug('send: host=%s, port=%s, data=%s', host, port, data.length);
...
UDP.prototype.sendR = function (host, port, data, callback) {
  ...
  var dataList = utils.splitBuffer(data, this._options.maxMessageSize);
  var session = this._getSessionId();
  this._debug('sendR: host=%s, port=%s, session=%s, data=%s', host, port, session, data.length);
  if (dataList.length > 1) {
    this._sendRMulti(host, port, session, dataList, callback);
  } else {
    this._sendRSingle(host, port, session, data, callback);
  }
};

UDP.prototype._sendRSingle = function (host, port, session, data, callback) {
...
UDP.prototype._sendRMulti = function (host, port, session, dataList, callback) {
  var debug = this._debug;
  var socket = this._socket;
  var timestamp = utils.getSecondTimestamp();
